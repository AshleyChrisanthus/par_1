...
class TennisBallMarker(Node):
    def __init__(self):
        ...
        # Camera intrinsics — replace these with real values from /oak/rgb/camera_info
        self.fx = 615.0
        self.fy = 615.0
        self.cx = 320.0
        self.cy = 240.0
        self.camera_frame = 'oak_rgb_camera_frame'  # replace with correct TF frame
        ...
        self.get_logger().info('Tennis Ball Detector with Depth initialized!')
    ...
    def process_macadamia_nut(self, cv_image, ball):
        ...
        try:
            ...
            if self.tf_buffer.can_transform('map', point_camera.header.frame_id, rclpy.time.Time(), timeout=timeout):
                ...
                if self.is_new_ball(point_map):
                    ...
                    self.get_logger().info(f'Macadamia nut at map: ({point_map.point.x:.2f}, {point_map.point.y:.2f})')
                    self.home_trigger_pub.publish(Empty())
            else:
                self.get_logger().warn('TF transform to map not available')
        except Exception as e:
