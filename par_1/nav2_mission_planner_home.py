...
class Nav2MissionPlanner(Node):
    ...
    def _pose_from_xyyaw(self, x: float, y: float, yaw_deg: float) -> PoseStamped:
        ...
        pose = PoseStamped(header={'frame_id': 'map', 'stamp': self.get_clock().now().to_msg()})
    ...
    def _start_return_home_sequence(self):
        ...
        # --- THE HUMBLE-COMPATIBLE FIX ---
        # Create a regular timer that we will cancel manually inside its callback
        self.return_timer = self.create_timer(10.0, self._send_home_goal)
...
def main(args=None):
    ...
    try:
        ...
        # This will handle shutdown more gracefully
        if rclpy.ok() and 'node' in locals() and node.executor:
            node.destroy_node()
        if rclpy.ok():
            rclpy.shutdown()
