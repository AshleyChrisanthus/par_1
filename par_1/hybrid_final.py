...
class HybridTennisBallDetector(Node):
    def __init__(self):
        ...
        # Force the node to use system time to prevent conflicts
    ...
    def process_tennis_ball(self, msg, ball_data):
        point_in_map_frame = self.calculate_3d_position(msg, ball_data)
        if point_in_map_frame and self.is_new_ball(point_in_map_frame):
            self.detection_count += 1
            self.get_logger().info(f'✓ NEW DETECTION! Total: {self.detection_count} | Map Coords: X={point_in_map_frame.point.x:.2f}, Y={point_in_map_frame.point.y:.2f}, Z={point_in_map_frame.point.z:.2f}')
    ...
    def calculate_3d_position(self, msg, ball_data):
        target_frame = 'map'
        
        x, y, w, h = ball_data['bbox']
        depth_mm, fx, fy, cx, cy = (ball_data['depth'], self.camera_intrinsics.k[0], self.camera_intrinsics.k[4], self.camera_intrinsics.k[2], self.camera_intrinsics.k[5])
        pixel_x, pixel_y, depth_m = x + w / 2, y + h / 2, depth_mm / 1000.0
        ...
        try:
            # Step 1: Look up the transform valid for the LATEST available time.
            # This is robust against stale TF data from a stationary robot.
            transform = self.tf_buffer.lookup_transform(target_frame, self.source_frame, rclpy.time.Time())
            
            # Step 2: Apply that transform to our point.
            point_in_map_frame = tf2_geometry_msgs.do_transform_point(point_in_camera_frame, transform)
    ...
    def is_new_ball(self, new_point, threshold=0.3):
        for point in self.detected_points:
            dx, dy, dz = (new_point.point.x - point.point.x, new_point.point.y - point.point.y, new_point.point.z - point.point.z)
            if np.sqrt(dx*dx + dy*dy + dz*dz) < threshold: return False
        return True
    ...
    def publish_marker(self, point_map):
        ...
        marker.ns, marker.id, marker.type, marker.action = 'macadamia_nuts', self.detection_count, Marker.SPHERE, Marker.ADD
    ...
    def create_debug_visualization(self, cv_image, detected_balls):
        ...
        for ball_data in detected_balls:
            x, y, w, h = ball_data['bbox']
            label = f"Ball @ {ball_data['depth'] / 1000.0:.2f}m"
...
def main(args=None):
    ...
    try:
        ...
        if detector: detector.destroy_node()
        rclpy.shutdown()
